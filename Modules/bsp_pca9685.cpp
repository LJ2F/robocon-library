...
uint8_t pca9685::calc_prescale(float freq_hz) noexcept {
  ...
  const float prescale_f =
      (internal_oscillator_hz / (static_cast<float>(resolution) * freq_hz)) - 1.0f;
...
HAL_StatusTypeDef pca9685::write_register(uint8_t reg_addr, uint8_t value) {
  ...
  return m_bus->mem_write(m_dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT,
                          &value, 1, std::chrono::milliseconds(20));
}
...
HAL_StatusTypeDef pca9685::read_register(uint8_t reg_addr, uint8_t& value) {
  ...
  return m_bus->mem_read(m_dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT,
                         &value, 1, std::chrono::milliseconds(20));
}

HAL_StatusTypeDef pca9685::write_registers(uint8_t reg_addr, const uint8_t* data,
                                           uint16_t size) {
  ...
  return m_bus->mem_write(m_dev_addr, reg_addr, I2C_MEMADD_SIZE_8BIT,
                          data, size, std::chrono::milliseconds(20));
}

HAL_StatusTypeDef pca9685::update_mode1_bits(uint8_t set_mask, uint8_t clear_mask) {
  uint8_t mode1 = 0;
  HAL_StatusTypeDef status = read_register(static_cast<uint8_t>(reg::mode1), mode1);
  if (status != HAL_OK) {
    return status;
  }

  mode1 = static_cast<uint8_t>((mode1 | set_mask) & static_cast<uint8_t>(~clear_mask));
...
HAL_StatusTypeDef pca9685::sleep() {
  return update_mode1_bits(mode1_sleep, 0);
}
...
HAL_StatusTypeDef pca9685::set_pwm_freq(float freq_hz) {
  ...
  uint8_t old_mode1 = 0;
  HAL_StatusTypeDef status = read_register(static_cast<uint8_t>(reg::mode1), old_mode1);
  if (status != HAL_OK) {
    return status;
  }
  ...
  uint8_t sleep_mode =
      static_cast<uint8_t>((old_mode1 & static_cast<uint8_t>(~mode1_restart)) | mode1_sleep);
  ...
  // RESTART + 保持 AI
  status = write_register(static_cast<uint8_t>(reg::mode1),
...
HAL_StatusTypeDef pca9685::set_duty(std::uint8_t channel, std::uint16_t duty, bool invert) {
  if (channel >= channel_count) {
    return HAL_ERROR;
  }
...
//度数接口
HAL_StatusTypeDef pca9685::set_servo_angle(std::uint8_t channel, float angle_deg,
                                           float min_pulse_us,
                                           float max_pulse_us,
                                           float max_angle_deg) {
  if (channel >= channel_count) {
  ...
  const float pulse_us =
      min_pulse_us + (angle_deg / max_angle_deg) * (max_pulse_us - min_pulse_us);
