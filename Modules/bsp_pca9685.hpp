...
public:
  ...
  explicit pca9685(gdut::i2c& bus, std::uint16_t dev_addr = default_address) noexcept
  ...
  // 通道原始 PWM 设置
  HAL_StatusTypeDef set_pwm(std::uint8_t channel, std::uint16_t on, std::uint16_t off);

  // 直接按 0~4095 设置占空值
  HAL_StatusTypeDef set_duty(std::uint8_t channel, std::uint16_t duty, bool invert = false);
...
private:
  ...
  HAL_StatusTypeDef update_mode1_bits(std::uint8_t set_mask, std::uint8_t clear_mask);
